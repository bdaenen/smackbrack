...
import smackbrackApi from '../../api/smackbrack';
...
export default class Create extends React.Component {
    ...
    handleSubmit = async e => {
        ...
        if (type === 'swiss' && !rounds) {
            errors.push(
                'Rounds is a required field and should be greater than 0'
            );
        }


        if (!errors.length) {
          let board = await smashtrackApi.createBoard(name, stages, users);

          if (!board) {
            errors.push('Something went wrong while saving to SmashTrack. Please try again in a while or contact benno.daenen+smashtrack@gmail.com if the problem persists.');
          }
          else {
              console.log(board)
            smackbrackApi.createTournament(board.board.id, {
                type,
                name,
                rounds
            })
                            ...
                            <Row>
                                ...
                                <Col>
                                    ...
                                    <Select.TournamentType
                                        onChange={
                                            this.handleTournamentTypeChange
                                        }
                                        id="typeSelect"
                                    />
                                </Col>
